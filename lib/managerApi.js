...
async function listPublicPopups(options = {}) {
  ...
  const response = await request('/api/stores/v1/popups', {
    auth: false,
    query: { page, size, category, storeId, keyword, regionId }
  });
...
async function refreshAccessToken() {
  ...
  refreshPromise = (async () => {
    ...
    let response;
    try {
      response = await fetch(buildRequestUrl('/api/users/v1/auth/refresh'), {
        method: 'POST',
        headers: {
  ...
  if (auth) {
    ...
    if (isAccessTokenExpiringSoon(authToken) && refreshToken) {
      try {
        ...
        // Only clear session when refresh token exists but refresh actually fails.
        // If no refresh token strategy is used by backend, keep current session.
...
export async function listStores() {
  try {
    ...
    const canFallbackForManager = (
      isManagerSession()
      && isApiError(error)
      && [403, 404, 500, 503].includes(error.status)
    );

    if (!canFallbackForManager) {
      throw error;
    }

    const publicPopups = await listPublicPopups({ page: 1, size: 200 });
...
export async function listCoupons() {
  ...
  return request('/api/v1/coupons/all');
}
...
export function mapOrderItemToUiOrder(item) {
  return {
    ...
    customerName: `고객 #${item.userId}`,
    products: [makeOrderProductLabel(item)],
    totalAmount: toNumber(item.linePrice),
    status: mapSnapshotStatusToUi(item.orderStatus, item.paymentStatus),
    rawOrderStatus: item.orderStatus,
...
export async function getDashboardHealth() {
  return request('/api/orderquery/v1/dashboard/health');
}
...
export async function getDashboardMain(baseDate = null) {
  ...
  return request('/api/orderquery/v1/dashboard/main', { query });
}
...
export async function getDashboardMainByStore(storeId, baseDate = null) {
  const query = baseDate ? { baseDate } : {};
  return request(`/api/orderquery/v1/dashboard/stores/${storeId}/main`, { query });
}
...
export async function getDashboardStatusSummary() {
  return request('/api/orderquery/v1/dashboard/status-summary');
}
