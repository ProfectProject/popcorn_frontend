...
function extractErrorMessage(payload, status) {
  if (typeof payload === 'string' && payload.trim()) {
    ...
    const looksLikeHtml = lowered.includes('<html') || lowered.includes('<body') || lowered.includes('<!doctype');
    const hasTemporaryUnavailable = lowered.includes('service temporarily unavailable');
    const hasGatewayTimeout = lowered.includes('gateway timeout');

    if (looksLikeHtml || hasTemporaryUnavailable || hasGatewayTimeout) {
      if (status === 503) return '로그인 서버가 일시적으로 불안정합니다. 잠시 후 다시 시도해주세요.';
      if (status === 504) return '요청 시간이 초과되었습니다. 잠시 후 다시 시도해주세요.';
      if (status === 502) return '게이트웨이 연결에 실패했습니다. 잠시 후 다시 시도해주세요.';
